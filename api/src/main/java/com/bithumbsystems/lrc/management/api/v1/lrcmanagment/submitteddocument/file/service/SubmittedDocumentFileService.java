...
/**
 * The type Submitted document file service.
 */
@Slf4j
@Service
...
public class SubmittedDocumentFileService {
  ...
  public Mono<SubmittedDocumentFileResponse> saveAll(Mono<SubmittedDocumentFileRequest> fileRequest, Account account) {
    return fileRequest
        .flatMap(request -> DataBufferUtils.join(request.getFile().content())
                .flatMap(dataBuffer -> {
                  ...
                  return fileService.upload(fileKey, fileName, fileSize, awsProperties.getBucket(), buf)
                      ...
                      .publishOn(Schedulers.boundedElastic())
                      .flatMap(file -> {
                        historyLogSend(request.getProjectId(), "제출서류", request.getType(), "문서추가(관리자)",
                            Normalizer.normalize(fileName, Normalizer.Form.NFC), account);
                        ...
                        return submittedDocumentFileDomainService.save(
                                SubmittedDocumentFile.builder()
                                        ...
                                        .email(AES256Util.encryptAES(
                                            awsProperties.getKmsKey(), account.getEmail(), awsProperties.getSaltKey(), awsProperties.getIvKey()))
