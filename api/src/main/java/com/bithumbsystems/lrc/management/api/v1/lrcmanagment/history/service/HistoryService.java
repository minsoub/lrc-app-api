    ...
    return historyDomainService.findBySearch(projectId, keyword)
          ...
          if (result.getType() == null || result.getUpdateAccountId() == null) {
            ...
            if (result.getType().equals("ADMIN")) {
              ...
              return userAccountDomainService.findByProjectIdAndUserAccountId(projectId, result.getUpdateAccountId())
                  .flatMap(r1 -> {
                    String customer = (StringUtils.hasLength(r1.getName()))
                        ? MaskingUtil.getNameMask(AES256Util.decryptAES(awsProperties.getKmsKey(), r1.getName()))
                        + "(" + MaskingUtil.getEmailMask(result.getCustomer()) + ")" : MaskingUtil.getEmailMask(result.getCustomer());
        ...
        .filter(matchResult -> {
          if (!StringUtils.hasLength(keyword)) {
            ...
            return matchResult.getMenu().contains(keyword) || matchResult.getSubject().contains(keyword)
                || matchResult.getTaskHistory().contains(keyword) || matchResult.getCustomer().contains(keyword);
          }
        })
