...
public class ProjectInfoService {
  ...
  public Mono<ProjectInfoResponse> findByProjectId(String projectId) {
    return projectInfoDomainService.findByProjectId(projectId)
        .flatMap(result ->
            lineMngDomainService.findByIdWithParentInfo(result.getBusinessCode())
                .flatMap(r -> Mono.just(ProjectInfoResponse.builder()
                    .id(result.getId())
        ...
        .flatMap(res -> lineMngDomainService.findByIdWithParentInfo(res.getNetworkCode())
            .flatMap(c -> {
  ...
  public Mono<ProjectInfoResponse> updateProjectInfo(String projectId, ProjectInfoRequest projectInfoRequest, Account account) {
    ...
    return projectInfoDomainService.findByProjectId(projectId)
        .flatMap(c -> {
          // 변경 히스토리 추가
          if ((c.getBusinessCode() == null && projectInfoRequest.getBusinessCode() != null)
              || !Objects.equals(c.getBusinessCode(), projectInfoRequest.getBusinessCode())) {
            lineMngDomainService.findById(projectInfoRequest.getBusinessCode())
                .flatMap(res -> {
                  historyLog.send(projectId, "프로젝트 관리>프로젝트 정보", "사업 계열", "수정", res.getName(), account);
                  // projectInfoRequest.getBusinessCode(), account);
                  return Mono.just(res);
                }).subscribe();

          }
          if ((c.getNetworkCode() == null && projectInfoRequest.getNetworkCode() != null)
              || !Objects.equals(c.getNetworkCode(), projectInfoRequest.getNetworkCode())) {
            lineMngDomainService.findById(projectInfoRequest.getNetworkCode())
                .flatMap(res -> {
                  historyLog.send(projectId, "프로젝트 관리>프로젝트 정보", "네트워크 계열", "수정", res.getName(), account);
                  // projectInfoRequest.getNetworkCode(), account);
                  return Mono.just(res);
                }).subscribe();

          }
          if ((c.getWhitepaperLink() == null && projectInfoRequest.getWhitepaperLink() != null)
              || !Objects.equals(c.getWhitepaperLink(), projectInfoRequest.getWhitepaperLink())) {
          ...
          if (c.getCreateDate() == null && projectInfoRequest.getCreateDate() == null) {
            // no job
          } else if (c.getCreateDate() == null || !c.getCreateDate().equals(projectInfoRequest.getCreateDate())) {
            historyLog.send(projectId, "프로젝트 관리>프로젝트 정보", "최초 발행일", "수정",
                projectInfoRequest.getCreateDate() == null ? "" : projectInfoRequest.getCreateDate().toString(), account);
          }
          if ((c.getContractAddress() == null && projectInfoRequest.getContractAddress() != null)
              || !Objects.equals(c.getContractAddress(), projectInfoRequest.getContractAddress())) {
            historyLog.send(projectId, "프로젝트 관리>프로젝트 정보", "컨트랙트 주소", "수정", projectInfoRequest.getContractAddress(), account);
          }
        ...
        .switchIfEmpty(Mono.defer(() -> {
          if (StringUtils.hasLength(projectInfoRequest.getBusinessCode())) {
            lineMngDomainService.findById(projectInfoRequest.getBusinessCode())
                .flatMap(res -> {
                  historyLog.send(projectId, "프로젝트 관리>프로젝트 정보", "사업 계열", "등록", res.getName(), account);
                  // projectInfoRequest.getBusinessCode(), account);
                  return Mono.just(res);
                }).subscribe();
          ...
          if (StringUtils.hasLength(projectInfoRequest.getNetworkCode())) {
            lineMngDomainService.findById(projectInfoRequest.getNetworkCode())
                .flatMap(res -> {
                  historyLog.send(projectId, "프로젝트 관리>프로젝트 정보", "네트워크 계열", "등록", res.getName(), account);
                  // projectInfoRequest.getNetworkCode(), account);
                  return Mono.just(res);
                }).subscribe();
          ...
          if (!ObjectUtils.isEmpty(projectInfoRequest.getCreateDate())) {
